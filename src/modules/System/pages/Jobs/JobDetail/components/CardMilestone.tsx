...
const CardMilestone: React.FC<CardMilestoneProps> = () => {
  ...
  const onEditMilestone = (jobMilestone: JobMilestone) => {
    setOpen(true);
    setActionType('edit');
            ...
            >
              <Row gutter={[12, 12]} wrap={false} className="scrollbar overflow-x-auto overflow-y-hidden p-4">
                {loading
                  ...
                  : jobMilestones?.map((item, index) => (
                      <Col xxl={6} xl={8} key={`${item.id}_${index}`}>
                        <SortableList.Item id={item.id}>
                          <MilestoneItem data={item} fetchData={fetchMilestoneByJobId} onEdit={onEditMilestone}>
                            <SortableList.DragHandle />
                          </MilestoneItem>
                        </SortableList.Item>
                      </Col>
