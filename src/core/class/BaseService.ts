...
const authAxios: Axios = axios.create({
  ...
  paramsSerializer: {
    serialize: function (params) {
      const { page_number, page_size, ..._params } = params;
      page_number !== undefined && (_params.page_number = page_number);
      page_size !== undefined && (_params.page_size = page_size);
...
class BaseService {
  ...
  public callWithBody: CallWithBodyMethod = (method, url, body, options, useAuth = true, usePrefix = true) => {
    if (useAuth && this.useAuth) {
      return authAxios[method](usePrefix ? `${this.prefix}${url}` : url, body, options).then(
        ({ data }: AxiosResponse) => data
      );
    }

    return axiosBase[method](usePrefix ? `${this.prefix}${url}` : url, body, options).then(
      ({ data }: AxiosResponse) => data
    );
  };
