import { Col, Form, FormInstance, InputNumber, Modal, ModalProps, Row, Select, Tooltip } from 'antd';
...
import { CommonService, JobService, JobTaskService } from '@/core/services';
import { Action, Equipment, EquipmentSubTask, JobSubTaskEquipment } from '@/core/types';
import { deleteEquipmentSubTask } from '@/modules/System/pages/TemplateForm/template-form.slice';
import { CommonHelper, LogHelper } from '@/utils/helpers';
import { useJobTask } from '@/store';

type SubTaskEquipmentModalProps = ModalProps & {
  data: JobSubTaskEquipment[];
  onCancel: () => void;
  onUpdate: (data: JobSubTaskEquipment[], listId: string[]) => void;
};
...
const SubTaskEquipmentModal: React.FC<SubTaskEquipmentModalProps> = ({ open: isOpen, onCancel, data, onUpdate }) => {
  const { jobId, milestoneId, id } = useParams();
  const { t } = useTranslation(['message', 'button']);
  const { pathname } = useLocation();

  const [equipmentsSubTaskLocal, setEquipmentsSubTaskLocal] = useState<EquipmentSubTask[]>(data);
  const [listEquipmentInit, setListEquipmentInit] = useState<string[]>([]);
  const [idAutoAdd, setIdAutoAdd] = useState<string | null>(null);
  const [equipmentsInit, setEquipmentsInit] = useState<Equipment[]>([]);
  const [submitting, setSubmitting] = useState(false);
  const { updateEquipmentToSubTask, subTaskIdActive } = useJobTask();
  const fetchData = async () => {
    if (!subTaskIdActive || pathname.includes(pathNameTaskTemplate)) return;
    const res: any = await JobService.getEquipmentTask(jobId || '', milestoneId || '', id || '', subTaskIdActive);
    if (res.success) {
      const data =
      ...
      setEquipmentsSubTaskLocal(data);
      setListEquipmentInit(res.data?.map((item: any) => item.id) ?? []);
      updateEquipmentToSubTask(subTaskIdActive, data);
    }
  };
  useEffect(() => {
    if (subTaskIdActive) fetchData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [subTaskIdActive, equipmentsInit]);

  const { subTaskActive } = useAppSelector((state) => state.templateForm);
  const dispatch = useAppDispatch();

  const fetchEquipments = useCallback(async () => {
    if (!jobId || pathname.includes(pathNameTaskTemplate)) return;
    try {
      const res = await JobTaskService.getEquipmentsJob(jobId);
      if (res.success) {
    ...
    () =>
      defaultColumns.map((col) => {
        ...
        return {
          ...col,
          onCell: (record: any) => {
            if (record?.equipment_id && col.dataIndex === 'equipment_id') return;
            return {
              record,
  ...
  const onAddEquipment = async () => {
    try {
      ...
      setEquipmentsSubTaskLocal((pre) => [...pre, { id: CommonHelper.generateStr(), quantity: 0, serial_number: '' }]);
    } catch (error: any) {
  ...
  const onOk = async () => {
    ...
    try {
      await validateEquipmentSubTask();
      onUpdate(
        equipmentsSubTaskLocal?.filter((item) => item?.equipment_id) as JobSubTaskEquipment[],
        listEquipmentInit
      );
      onCancel();
    } catch (error: any) {
...
export default memo(SubTaskEquipmentModal);
